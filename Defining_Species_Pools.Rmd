...
adf_sim_func_island <- function(cells){
                  ...
                  mat <- matrix(rep(NA,ncol(map_data)), nrow = 1, ncol = ncol(map_data))
                              ...
                              if(!any(!is.na(as.numeric(map_data[,cells])))){
                                ...
                                coords <- coords[1,]
                                coords <- SpatialPoints(coords[,c("Longitude","Latitude")], proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs"))
...
#                       for(sit in sp_sites){
#                           
#                           s_coords <- SpatialPoints(coords[which(coords$SSBS == sit),c("Longitude","Latitude")], proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs"))
#                         
