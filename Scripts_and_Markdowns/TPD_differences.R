...
realm_land_use_loop <-
  function(FUN,
    ...
    for (r in names(realm_land_uses)) {
...
PREDICTS <- PREDICTS_full %>%  ## PREDICTS data
  dplyr::distinct(SSBS, Predominant_habitat, Use_intensity, Realm, SS,Biome) %>% ## pull out land_use type, Subregion, realm etc
  dplyr::mutate(
  ...
  ) %>% dplyr::filter(Predominant_habitat != "Primary non-forest") %>% data.frame()
                  ...
                  c("Primary vegetation",
                    ...
                    realm_land_uses[["Australasia"]] <-
                      c("Primary vegetation",
                        ...
                        cell_probability_of_occupancy <-
                          realm_land_use_loop(
                            FUN = cell_probability_of_occupancy_function,
                            objects = c("vector"),
                        ...
                        legend_col <-
                          as.numeric(quantile(cell_probability_of_occupancy, seq(0.0002, 1, 0.0002)))
                        ...
                        mapping_lists <-
                          realm_land_use_loop(
                            ...
                            objects = c("list", "list"),
                            object_names = c("TPD_for_mapping_data_random",
                        ...
                        1- mean(dietary_niche_accuracy$unassigned_cells)
