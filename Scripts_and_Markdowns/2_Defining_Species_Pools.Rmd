                               ...
                               if(nrow(data) == 0){
                                 ...
                                 ### iterate through sites
                                  ...
                                  ### if Overlap = 0 add a 1 to the site indicating presence else a 0
                                   ...
                                   mat[,1] <- ifelse(st_intersects(shape,site_coords, sparse = FALSE )[1,], 1, 0)
 ...
 map_data <- foreach(range = species_files,
                     ...
                     .inorder = FALSE) %dopar% {
                        ...
                        if(nrow(data) == 0){
                          ...
                          ras <- fasterize::fasterize(sf = shape, raster = blank, fun = "count", background = 0)
...
for(i in 1:nrow(site_cell_matrix)){
  ...
  dir <- paste("../Outputs/assemblage_dispersion_fields/",paste(study,i,sep = "_"), sep = "")
...
## new cells
  ...
  site_cell_matrix <- site_cell_matrix[!duplicated(rownames(site_cell_matrix)),]
  write_rds(file = "../Outputs/predicts_site_cell_matrix.rds", x = site_cell_matrix)
  
  sites <- sites %>% dplyr::mutate(problem_cells = ifelse(new_cell %in% problem_cells, TRUE, FALSE))
...
if(any(grepl(list.files("Outputs"), pattern = "site_sp_distance"))){
  site_spp_dist <- readRDS(grep(list.files("Outputs", full.names = TRUE), pattern = "site_sp_distance", value = TRUE))
} else {
...
#### if any are still unvalid you will be unable to combine the polygons therefore you have to turn off S2 geography - this is GIS stuff I don't quite understand it just makes it work but with a but of a time penalty and some caveats so only going to use when absolutely necessary
  ...
  if(!st_is_valid(shape)){
    sf_use_s2(FALSE)
                       ...
                       s_coords <- coords %>% dplyr::filter(SSBS %in% sp_sites) %>% dplyr::select(-SSBS)
                           
                           s_coords <- st_as_sf(SpatialPoints(s_coords, proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs")))
                         
                           
                           as.numeric(st_distance(x = s_coords,y = shape))
                           
                           mat[sp_sites,sp] <- as.numeric(st_distance(s_coords,shape))
...
plot(1-sapply(seq(0,1,0.01),function(x) pgamma(q = x,shape = 0.1)))
...
crosswalk <- read.csv("../../PhD_Code/PREDICTS_Taxonomy/BL_Jetz crosswalk v3.csv")
...
sp <- gsub(x = gsub(x= SDM_sp, replacement = "", pattern = "\\.rds"), pattern = "_", replacement = " ")

sdm_species <- data.frame(Birdlife_Name = sp)



resolve_species <- read.csv("../../Datasets/Bird_SDM_Projections_Probability/Miss_sp.csv")
...
assembly_species <- c()
for(i in 1:length(species_pools)){
  assembly_species <- unique(c(assembly_species, species_pools[[i]][["0.9"]]))
}




sites <- rownames(site_spp_dist)

environmental_probabilities <- matrix(rep(NA,length(sites)*length(assembly_species)), nrow = length(sites), ncol = length(assembly_species))
rownames(environmental_probabilities) <- sites
colnames(environmental_probabilities) <- assembly_species
...
for(sp in assembly_species){
print(sp)  
  ## if the species was missing from the SDM data given 
  
    if(sp %in% resolve_species$Birdlife_Name){
    
    ## get that species data
      
    res_dat <- resolve_species %>% dplyr::filter(Birdlife_Name == sp) 
    ...
    if(res_dat$DROP){
      ...
      sp_poly <- readRDS(grep(species_files, pattern = paste("/",sp,"Birdlife",sep = ""), value = TRUE))
  
      ## filter ranges 
      
      sp_poly <- sp_poly %>% dplyr::filter(presence %in% c(1,2,3), origin %in% c(1,2,3), seasonal %in% c(1,2,3))
                       
                       ##### funny thing with some ranges so need to convert back to type MULTIPOLYGON
 
                       if(any(class(sp_poly$Shape)[1] == "sfc_MULTISURFACE", class(sp_poly$Shape)[1] == "sfc_GEOMETRY")){
                         for(k in 1:NROW(sp_poly)){
                           sp_poly$Shape[[k]] <- st_cast(sp_poly$Shape[[k]], "MULTIPOLYGON")
                         }
                       }
 
      #combine

  sp_shape <- st_combine(sp_poly$Shape)
  
  ## give crs
  
  st_crs(sp_shape) <- "+proj=longlat +datum=WGS84 +no_defs"
  ...
  ## what sites does this species turn up in the species pool 
   ...
   sp_sites <- PREDICTS %>% dplyr::filter(SSBS %in% sp_sites) %>% distinct(SSBS,Longitude,Latitude)
   
   ## spatial points
   
   site_coords <- st_as_sf(SpatialPoints(sp_sites[,c("Longitude","Latitude")], proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs")))
   
   ## if the site overlaps with the species range code as 1 else 0
    
   environmental_probabilities[site_find(sp),sp] <- ifelse(st_intersects(sp_shape,site_coords, sparse = FALSE )[1,],1,0)
  ...
  next()
    ...
    if(!res_dat$DROP & res_dat$potential_species != "Mean"){
      ...
      sp_file <- grep(list.files("../../Datasets/Bird_SDM_Projections_Probability/", full.names = TRUE), pattern = paste(gsub(res_dat$potential_species, pattern = " ", replacement = "_"),".rds", sep = ""), value = TRUE,ignore.case = FALSE)
    ...
    ###### if it is the mean species then get the sister species from the crosswalk and those that SDM data is available.
      ...
      split_sp <- crosswalk %>% dplyr::filter(BirdLife.name == sp) %>% pull(Jetz.name)
      split_sp <- crosswalk %>% dplyr::filter(Jetz.name == split_sp) %>% pull(BirdLife.name)
      split_sp <- split_sp[which(split_sp %in% sdm_species$Birdlife_Name)]
   ...
   s_coords <- coords %>% dplyr::filter(SSBS %in% sp_sites) %>% dplyr::select(-SSBS) %>% SpatialPoints(proj4string = CRS("+proj=longlat +datum=WGS84 +no_defs")) %>% st_as_sf() %>% st_transform(crs = "+proj=cea +lat_ts=30")
     ...
     for(j in 1:length(split_sp)){
       
       # load the map
       
       sp_file <- grep(list.files("../../Datasets/Bird_SDM_Projections_Probability/", full.names = TRUE), pattern = paste(gsub(split_sp[j], pattern = " ", replacement = "_"),".rds", sep = ""), value = TRUE,ignore.case = FALSE)
       
      data <- readRDS(sp_file)
  
      # make raster
      
      sp_ras <- rasterFromXYZ(xyz = data[,c("lon","lat","current")], crs ="+proj=cea +lat_ts=30") 
     ...
     means <- rowMeans(mean_vals, na.rm = TRUE)
     
     environmental_probabilities[sp_sites,sp] <- means
  ...
  sp_file <- grep(list.files("../../Datasets/Bird_SDM_Projections_Probability/", full.names = TRUE), pattern = paste(gsub(sp, pattern = " ", replacement = "_"),".rds", sep = ""), value = TRUE,ignore.case = FALSE)
  }
  ...
  sp_ras <- rasterFromXYZ(xyz = data[,c("lon","lat","current")], crs ="+proj=cea +lat_ts=30")
   ...
   environ_p <- extract(x = sp_ras, y = s_coords)
     ...
     environmental_probabilities[sp_sites,sp] <- extract(x = sp_ras, y = s_coords)
  ...
  sp_sites <- PREDICTS %>% dplyr::filter(SSBS %in% zero_site) %>% distinct(SSBS,Longitude,Latitude)
   ...
   environmental_probabilities[zero_site,sp] <- ifelse(st_intersects(sp_shape,site_coords, sparse = FALSE )[1,],1,0)
