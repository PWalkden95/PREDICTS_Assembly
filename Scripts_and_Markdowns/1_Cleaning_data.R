...
PREDICTS_Aves <-
  readRDS("../PREDICTS_Taxonomy/PREDICTS_BL_traits_out.rds")
...
keep <-
  c("FE1_2015__Marinaro",
    "JB3_2013__Gove",
    "DL1_2011__Latta",
    "DB1_2010__Dures")

PRED_Assem <-
  PREDICTS_Aves %>% dplyr::filter(Diversity_metric_type == "Abundance", Measurement > 0) %>% dplyr::group_by(SS) %>%
  dplyr::mutate(spp_num = n_distinct(Birdlife_Name)) %>% dplyr::filter(
    spp_num > 1,
    grepl(Study_name, pattern = "bird", ignore.case = TRUE) |
      Sampling_target == "Entire community" | Source_ID %in% keep
  ) %>%
  droplevels()
...
refine_data <-
  PRED_Assem %>% filter(!(Source_ID %in% drop$study)) %>% filter(SS != "GN1_2010__Hvenegaard 2")
...
refine_data <- refine_data %>%
  ...
  dplyr::mutate(
    Effort_Corrected_Measurement = ifelse(
      Diversity_metric_is_effort_sensitive == TRUE,
...
retain <-
  c("MJ1_2016__Jung", "XH1_2015__Rachakonda")  ### each of the studies are distinct sites
...
refine_data <-
  refine_data %>% dplyr::mutate(
    Use_intensity = ifelse(
      SS == "DL1_2009__Azpiroz 1" & Predominant_habitat == "Cropland",
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "DL1_2010__Proenca 2" &
        Predominant_habitat == "Plantation forest",
      "Minimal use",
      paste(Use_intensity)
    ),
    ...
    Use_intensity = ifelse(
      SS == "DL1_2012__Dallimer 1" &
        Predominant_habitat == "Secondary vegetation (indeterminate age)",
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SSBS %in% c("TN1_2006__Soh 1 Cameron 1", "TN1_2006__Soh 1 Fraser 6") ,
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "AD1_2012__Yamaura 1" & Predominant_habitat == "Pasture",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "AD1_2012__Yamaura 1" &
    ...
    Use_intensity = ifelse(
      SS == "GN1_2010__Hvenegaard 1" & Predominant_habitat == "Cropland",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SSBS %in% c("HP1_2008__Gomes 1  1", "HP1_2008__Gomes 1  9"),
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SSBS %in% c("HP1_2008__Gomes 1  5", "HP1_2008__Gomes 1  6"),
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "HP1_2008__Gomes 1" &
        Predominant_habitat == "Secondary vegetation (indeterminate age)",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(SS == "SH1_2014__Walker 2",
                           "Minimal use", paste(Use_intensity)),
    Use_intensity = ifelse(
      SS == "MJ1_2013__Ndanganga 1" &
        Predominant_habitat %in% c("Cropland", "Pasture"),
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "GN1_2018__AlvarezAlvarez 1" &
        Predominant_habitat == "Cropland",
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "GN1_2018__Karp 1" & Predominant_habitat == "Pasture",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "HW1_2007__Chapman 1" &
        Use_intensity == "Cannot decide" & Predominant_habitat == "Urban",
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "HW1_2007__Chapman 1" & Use_intensity == "Cannot decide",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "HW1_2011__Cerezo 1" & Predominant_habitat == "Cropland",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "HW1_2011__Cerezo 1" & Predominant_habitat == "Pasture",
      "Intense use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "MJ1_2008__Munyekenye 1" &
        Predominant_habitat == "Primary forest",
      "Minimal use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
    ...
    Use_intensity = ifelse(
      SS == "DL1_2009__Azpiroz 1" & Predominant_habitat == "Pasture",
      "Light use",
      paste(Use_intensity)
    ),
    Use_intensity = ifelse(
      SS == "MJ1_2013__Reynolds 1" &
