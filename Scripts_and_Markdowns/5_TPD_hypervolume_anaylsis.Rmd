...
PREDICTS_tpds <-
  readRDS("../Outputs/PREDICTS_sites_tpds.rds") # morphometric TPDs of 


PREDICTS_randomisations <-
  readRDS("../Outputs/randomisations_TPD_morpho.rds") ## morphometric TPD of r
...
In the Afrotropics we can compare Cropland, Pasture, Plantation forest, Primary vegetation,  and Secondary vegetation.

In Australasia we can compare Pasture, Primary vegetation,  and Secondary vegetation.

In Indo-Malay we can compare Cropland, Plantation forest, Primary vegetation, and Secondary vegetation.

In the Nearctic we can compare Cropland, Pasture, Primary vegetation,  and Secondary vegetation.

In the Neotropics we can compare Cropland, Pasture, Plantation forest, Primary vegetation, and Secondary vegetation.

In the Palearctic we can compare Cropland, Pasture, Plantation forest, Primary vegetation,  and Secondary vegetation.

Primary vegetation is the only land-use classification that is well represented ubiquitously across the realms, but there is also good coverage from secondary vegetation, plantation forest, pasture and, maybe, cropland.

Urban and  classifications are represented well in only three of the realms... maybe a case for combining primary sites together??

However, for now lets have a look at these trait spaces and use Primary vegetation sites in the Afrotropics as an example.
...
if (any(grepl(list.files("../Outputs"), pattern = "site_level_FD_model_data"))) {
  ...
  for (r in realms) {
    ...
    for (LU in land_uses) {
      ...
      metrics <-
        TPD_FD_metrics(data = PREDICTS_tpds, sites = lu_sites)
...
plot_data <- plot_data %>% dplyr::filter(Realm %in% realms)
...
hist(car::logit(plot_data$FDis))
plot_data$FDis <- car::logit(plot_data$FDis)

mod_rich <-
  lmer(sqrt(FRich) ~ Land_use * Realm + (1| SS) , data = plot_data)
...
rich_post_hoc <- test <-
  lsmeans(mod_rich, pairwise ~ Land_use * Realm, adjust = "tukey")


rich_contrasts <- summary(rich_post_hoc)[["contrasts"]]
...
mix_model_boots <-
  mix_model_boot(
    ...
    transformation = c("sqrt", "logit", "none"),
    newdata = pred_mod
  )
...
```{r general model trends}

median_value <- c()
for(r in realms){
  
  for(LU in land_uses[-1]){
    
    
    data <- data.frame(land_use = LU, difference = median(mix_model_boots$value[mix_model_boots$Land_use == LU & mix_model_boots$Realm == r & mix_model_boots$model == "FEve"])/median(mix_model_boots$value[mix_model_boots$Land_use == "Primary vegetation" & mix_model_boots$Realm == r & mix_model_boots$model == "FEve"]))
    
    median_value <- rbind(median_value,data)
...
for(LU in land_uses[-1]){
  ...
  print(mean(median_value$difference[median_value$land_use == LU], na.rm = TRUE) - 1)
...
site_level_beta <- readRDS("../Outputs/sites_tpd_beta_diversity_full_range.rds")




if(!any(grepl(list.files("../Outputs"),pattern = "site_level_beta_data"))){
  
for(i in 1:nrow(site_level_beta)){
  
  site_1 <- site_level_beta$site1[i]
  site_2 <- site_level_beta$site2[i]

  
  site_level_beta$land_use_comp[i] <- paste(TPD_LU$Predominant_habitat[which(TPD_LU$SSBS == site_2)][1],TPD_LU$Predominant_habitat[which(TPD_LU$SSBS == site_1)][1], sep = "_")
    
}


geographical_distance_function <- function(dataframe){
  
  for(i in 1:nrow(dataframe)){
   
     site_one <- dataframe[i,"site1"]
    site_two <- dataframe[i,"site2"]
    
    coords <- PREDICTS_full %>% dplyr::filter(SSBS %in% c(site_one, site_two)) %>% dplyr::distinct(Longitude,Latitude)
  
    if(nrow(coords) == 1){
      dataframe[i,"geographical_distance"] <- 0
    } else {
    
    dataframe[i,"geographical_distance"] <- distHaversine(coords)
    }
    }
...
site_level_beta <-  geographical_distance_function(site_level_beta)
land_uses <- c("Primary vegetation", "Secondary vegetation", "Plantation forest", "Pasture", "Cropland", "Urban")


write_rds(x = site_level_beta, file = "../Outputs/site_level_beta_data_full_ranges.rds")
}


site_level_beta <- readRDS("../Outputs/site_level_beta_data.rds")
...
hist(log10(slb_data$geographical_distance))

slb_data$logit_beta <- car::logit(slb_data$beta)
slb_data$log10_geodist <- log10(slb_data$geographical_distance)
slb_data[is.infinite(slb_data$log10_geodist),"log10_geodist"] <- 0
...
beta_mod <-
  lmer(logit_beta ~ land_use_comp * Realm + log10_geodist + land_use_comp:log10_geodist + (1 | SS), data = slb_data)
...
test <-
  emmeans(beta_mod, pairwise ~ land_use_comp * Realm, adjust = "tukey")



pairwise_sig <- summary(test)[["contrasts"]]

length(unique(TPD_LU$SS))

```

```{r beta diversity figures }

if(!any(grepl(list.files("../Outputs"),pattern = "bootstrap_beta_models.rds"))){

beta_newdata <-
  slb_data %>% distinct(Realm, land_use_comp) %>% dplyr::select(Realm, land_use_comp)

beta_newdata$log10_geodist <- 0
...
write_rds(file = "../Outputs/bootstrap_beta_models_full_ranges.rds", x = beta_boot)
}

beta_boot <- readRDS("../Outputs/bootstrap_beta_models.rds")
...
```{r beta effect size plot}

for(r in realms){

  effect_data <- beta_boot %>% dplyr::filter(grepl(Land_use_comp, pattern = "Primary vegetation"), Realm == r)

beta_effect_plot <- ggplot(data = effect_data, aes(x = Land_use_comp, y = value)) +
  geom_boxplot()

plot(beta_effect_plot)
...
for(r in realms){
  for(LU in names(TPD_forage_mapped[[r]])){
  
    data <- TPD_forage_mapped[[r]][[LU]]
    data$colour <- ifelse(data$Trophic_niche == "Om", "orchid",paste(data$colour))
    TPD_forage_mapped[[r]][[LU]] <- data
    
    data <- TPD_forage_mapped_random[[r]][[LU]]
    data$colour <- ifelse(data$Trophic_niche == "Om", "orchid",paste(data$colour))
    TPD_forage_mapped_random[[r]][[LU]] <- data
...
## then we can plot this out so we can visulaise how the dietary guilds are distributed across trait space, keeping the same example as the previous plot lets have a look at Primary vegetation in the Afrotropics
...
full_redundancy_frame <- c()

for(realm in names(TPD_forage_mapped)){
  
  for(land_use in names(TPD_forage_mapped[[realm]])){
    
    tpd_for_map <- TPD_forage_mapped[[realm]][[land_use]]

    redundancy_frame <- tpd_for_map %>% dplyr::group_by(Trophic_niche) %>% dplyr::summarise(redundancy = sum(prob)) 
    
    redundancy_frame$realm <- realm
    redundancy_frame$land_use <- land_use
    
    
    full_redundancy_frame <- rbind(full_redundancy_frame,redundancy_frame)
...
mean_redundancy <- full_redundancy_frame %>% 
  group_by(Trophic_niche,land_use) %>% 
  dplyr::summarise(redundancy = mean(redundancy)) %>%
  dplyr::mutate(
    land_use = factor(land_use, levels = c("Primary vegetation", "Secondary vegetation", "Plantation forest", "Pasture", "Cropland", "Urban")),
    ...
    colour = ifelse(Trophic_niche == "Om", "orchid", colour)
  ) %>%
...
redundancy_plot <-
  ggplot(data = mean_redundancy, aes(x = land_use, y = redundancy, group = Trophic_niche)) +
  geom_line(show.legend = FALSE,
            lwd = 1,
            colour = mean_redundancy$colour) +
  geom_point(
    show.legend = FALSE,
    size = 3,
    colour = "black",
    shape = 21,
    fill = mean_redundancy$colour
  ) + 
  scale_y_continuous(limits = c(-0.085,0.45),breaks = c(0,0.15,0.3,0.45)) +
...
redundancy_model_frame <- full_redundancy_frame %>% dplyr::filter(Trophic_niche %in% c("Om","In","Fr","Ne","Gr")) %>%
  dplyr::mutate(land_use = factor(land_use, levels = land_uses),
                Trophic_niche = relevel(factor(Trophic_niche), ref = "In"))



hist(sqrt(redundancy_model_frame$redundancy))

redundancy_model <- glm(sqrt(redundancy) ~ Trophic_niche + land_use + Trophic_niche:land_use, data = redundancy_model_frame)


summary(redundancy_model)
car::Anova(redundancy_model)

pairwise_redundancy_model <-
  lsmeans(redundancy_model, pairwise ~ land_use * Trophic_niche, adjust = "tukey")

pairwise_contrasts <- summary(pairwise_redundancy_model)$contrasts
...
if(any(grepl(list.files("../Outputs"), pattern = "completeness_list"))){
  prop_occ_list <- readRDS("../Outputs/completeness_list.rds")
} else {
...
write_rds(x = prop_occ_list, file = "../Outputs/completeness_list.rds")
...
prop_occ_plot_df <- prop_occ_plot_df %>% dplyr::filter(Trophic_niche != "Total")



for(niche in unique(prop_occ_plot_df$Trophic_niche)){
  
prop_occ_plot_df$mean_sim[which(prop_occ_plot_df$Trophic_niche == niche)] <-   prop_occ_plot_df$mean_sim[prop_occ_plot_df$Trophic_niche == niche]/prop_occ_plot_df$mean_sim[prop_occ_plot_df$Trophic_niche == niche & prop_occ_plot_df$land_use == "Primary vegetation"]
...
png(
  filename = 
    "../Figures/plots_for_figures/proportional_occupancy.png",
  width = 200,
  height = 150,
  units = "mm",
  res = 500
...
plot_df <- plot_df %>% dplyr::filter(Trophic_niche != "Total")

plot_df$Trophic_niche <- factor(plot_df$Trophic_niche)
plot_df$Trophic_niche <-
  relevel(plot_df$Trophic_niche, ref = "In")
...
test_mod <-
  glm(scale(car::logit(similarity))[,1] ~ land_use + Trophic_niche, data = plot_df)
