...
pri_afro <-
  TPD_LU %>% dplyr::filter(Predominant_habitat == "Primary forest",
...
plot_data$Realm <- relevel(plot_data$Realm, ref = "Australasia")
...
mix_boot_plot_FD <- function(boot_data) {
  ...
  levels(boot_data$Land_use) <-
    c("Urb", "Crp", "Pas", "PlnFor", "SecVeg","PriNFor", "PriFor")
  ...
  for (r in c("Palearctic",
    ...
    pri_intercept_rich <-
      median(mod_rich_plot %>% dplyr::filter(Land_use == "PriFor") %>% pull(value))
    ...
    pri_intercept_dis <-
      median(mod_dis_plot %>% dplyr::filter(Land_use == "PriFor") %>% pull(value))
    ...
    pri_intercept_eve <-
      median(mod_eve_plot %>% dplyr::filter(Land_use == "PriFor") %>% pull(value))
...
for(i in 1:nrow(site_level_beta)){
  ...
  site_level_beta$land_use_comp[i] <- paste(TPD_LU$Predominant_habitat[which(TPD_LU$SSBS == site_2)],TPD_LU$Predominant_habitat[which(TPD_LU$SSBS == site_1)], sep = "_")
...
land_uses <- c("Primary forest","Primary non-forest", "Secondary vegetation", "Plantation forest", "Pasture", "Cropland", "Urban")


write_rds(x = site_level_beta, file = "../Outputs/site_level_beta_data.rds")
...
slb_data$land_use_comp <-
  relevel(slb_data$land_use_comp, ref = "Primary forest_Primary forest")
...
write_rds(file = "../Outputs/bootstrap_beta_models.rds", x = beta_boot)


beta_levels <- c(
  "Primary forest_Primary forest",
  "Primary forest_Primary non-forest",
  "Primary forest_Secondary vegetation",
  "Primary forest_Plantation forest",
  "Primary forest_Pasture",
  "Primary forest_Cropland",
  "Primary forest_Urban",
  "Primary non-forest_Primary non-forest",
  "Primary non-forest_Secondary vegetation",
  "Primary non-forest_Plantation forest",
  "Primary non-forest_Pasture",
  "Primary non-forest_Cropland",
  "Primary non-forest_Urban",
...
map_data <- TPD_forage_mapped[["Neotropic"]][["Primary forest"]]
...
COM_coords <- COM_point_df %>%
  dplyr::mutate(
    colour = ifelse(Land_use == "Primary forest", "green4", NA),
    colour = ifelse(Land_use == "Primary non-forest", "chartreuse4", NA),
...
COM_mod_data <-
  COM_coords %>% dplyr::mutate(
    ...
    Land_use = relevel(Land_use, ref = "Primary forest")
  )
...
COM_group_mod$Land_use <-
  relevel(COM_group_mod$Land_use, ref = "Primary forest")
...
plot_df$land_use <-
  relevel(plot_df$land_use, ref = "Primary forest")
